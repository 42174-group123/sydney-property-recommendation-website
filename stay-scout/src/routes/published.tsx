...
function PublishedPage() {
  ...
  const fetchPublished = useServerFn(getPublishedListings);
  const deleteListing = useServerFn(deleteMyListing);
  ...
  const removeMutation = useMutation({
    mutationFn: (listing_id: string) => deleteListing({ data: { listing_id } }),
    onSuccess: () => {
    ...
    return (
      <div className="min-h-screen bg-secondary">
        <LoginGateModal
          ...
          message="Please log in to view your published listings."
        />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-secondary py-10">
      <div className="mx-auto max-w-3xl px-6">
        ...
        <h1 className="text-3xl font-bold text-foreground">Published listings</h1>
